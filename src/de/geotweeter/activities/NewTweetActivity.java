...
public class NewTweetActivity extends Activity {
	...
	public void onCreate(Bundle savedInstanceState) {
		useTwitpic = getSharedPreferences(Constants.PREFS_APP, 0).getString(
				"pref_image_hoster", "twitter").equals("twitpic");
		...
		editTweetText.setPlaceholder(Pattern
				.compile("http://twitpic\\.com/pic(\\d{3})"));
		...
		if (useTwitpic) {
			...
			/*
			 * Diese Funktionen werden benötigt, um Modifikationen von
			 * Twitpic-Platzhaltern durch den Benutzer zu unterbinden. Ist
			 * Twitter selbst als Bilderdienst eingestellt, muss hier gar nichts
			 * überwacht werden und wir können uns die Funktionen zur Laufzeit
			 * komplett sparen
			 */
			...
			editTweetText.setOnKeyListener(new OnKeyListener() {
				...
				public boolean onKey(View v, int keyCode, KeyEvent event) {
					return tweetTextKeyListener(
							(ProtectedPlaceholderEditText) v, keyCode, event);
				}
			});

		}

		List<String> autocompletionList = new ArrayList<String>(Geotweeter
				.getInstance().getAutoCompletionContent());
		Collections.sort(autocompletionList,
				Utils.getAlphabeticalStringComparator());
		ArrayAdapter<String> completeAdapter = new ArrayAdapter<String>(this,
				android.R.layout.simple_dropdown_item_1line, autocompletionList);
		editTweetText.setAdapter(completeAdapter);
		editTweetText.setTokenizer(new SpaceTokenizer());
		editTweetText.setRawInputType(InputType.TYPE_CLASS_TEXT
		...
		((Button) findViewById(R.id.btnSend))
				.setOnClickListener(new SendTweetListener());
		...
		findViewById(R.id.btnNoDM).setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				...
				editTweetText.setText("@" + dmRecipient + " "
						+ editTweetText.getText().toString());
				editTweetText.setSelection(oldSelection
						+ (dmRecipient.length() + 2));
				dmRecipient = null;
	...
	/**
	 * Prevents modification of placeholder URLs with by selecting them
	 * completely
	 * 
	 * @param v
	 *            The EditText
	 ...
	 * @param event
	 *            The event which triggered the event
	 * @return true if the event was handled
	 */
	protected boolean tweetTextKeyListener(ProtectedPlaceholderEditText v,
			int keyCode, KeyEvent event) {
		if (event.getAction() == KeyEvent.ACTION_DOWN) {
			if (v.isPlaceholderSelected()) {
				v.setDpadAction(keyCode >= KeyEvent.KEYCODE_DPAD_UP
	...
	protected class AccountChangerListener implements OnClickListener {
		public void onClick(View v) {
			...
			if (acc != currentAccount) {
				/*
				 * TODO: Hole oldView auf anderem Weg. Map, die in 2 Richtungen
				 * funktioniert
				 */
	...
	public void addImageHandler(View v) {
		...
		List<ResolveInfo> listCam = getPackageManager().queryIntentActivities(
				captureIntent, 0);
	...
	protected void onActivityResult(int request_code, int result_code,
			Intent data) {
		if (request_code == PICTURE_REQUEST_CODE
			...
			String image_hoster = getSharedPreferences(Constants.PREFS_APP, 0)
					.getString("pref_image_hoster", "twitter");
			...
			if (image_index == -1) {
				...
				if (image_hoster.equals("twitpic")) {
					...
					editTweetText.getText().insert(
							editTweetText.getSelectionStart(),
							prefix
									+ TwitpicApiAccess
											.getPlaceholder(image_index) + " ");
					// editTweetText.append(prefix +
					// TwitpicApiAccess.getPlaceholder(imageIndex) + " ");
				}
	...
	public void imageManagerHandler(View v) {
		...
		GridView gridView = (GridView) vi
				.inflate(R.layout.image_gridview, null);
		...
		new AlertDialog.Builder(this)
				...
				.setPositiveButton(R.string.action_delete,
						new DialogInterface.OnClickListener() {
							...
							public void onClick(DialogInterface dialog,
									int which) {
								editTweetText.setText(imageAdapter
										.deleteAllMarkedPlaceholder(editTweetText
												.getText().toString()));
	...
	protected class TextChangedListener implements TextWatcher {
		...
		private String text;
		private int start;
		private Pattern findDMPattern = Pattern.compile(
				"^[dm] @?([a-z0-9_]+)\\s(.*)$", Pattern.CASE_INSENSITIVE
						| Pattern.DOTALL);
		...
		public void beforeTextChanged(CharSequence s, int start, int count,
				int after) {
			// Log.d(LOG, s + " start: " + start + " count: " + count +
			// " after: " + after);
			Pattern p = Pattern.compile("http://twitpic\\.com/pic(\\d{3})");
			Matcher matcher = p.matcher(s);
			while (matcher.find()) {
				// Log.d(LOG, "start=" + matcher.start() + " end=" +
				// matcher.end() + " " + (matcher.start() <= start && start <
				// matcher.end()) + " Image: " + matcher.group());
				if (!delete && start < matcher.end()) {
					boolean insertion = after > count;
					if ((insertion && matcher.start() < start)
							|| (!insertion && matcher.start() <= start)) {
						text = s.toString().replace(matcher.group(), "");
						this.start = matcher.start();
						delete = true;
						imageAdapter.deleteIndex(Integer.parseInt(matcher
								.group(1)));
						if (imageAdapter.getCount() == 1) {
							btnImageManager
									.setImageResource(R.drawable.picture);
		...
		public void afterTextChanged(Editable s) {
			if (delete) {
				...
				if (dmRecipient == null && dmMatcher.find()) {
					...
					int newSelectionStart = oldSelectionStart - oldLength
							+ newLength;
					if (newSelectionStart < 0)
						newSelectionStart = 0;
					if (newSelectionStart > s.length())
						newSelectionStart = s.length();
	...
	protected class GPSToggleListener implements OnCheckedChangeListener {
		public void onCheckedChanged(CompoundButton buttonView,
				boolean isChecked) {
			SharedPreferences prefs = getSharedPreferences(Constants.PREFS_APP,
					0);
	...
	protected class GPSCoordsListener implements LocationListener {
		...
		public void onLocationChanged(Location new_location) {
			/*
			 * Wir nehmen immer die aktuellen Koordinaten, wenn es a) die ersten
			 * Koordinaten sind oder b) die bisherigen Koordinaten nur
			 * Netzwerk-genau waren oder c) wir aktuell GPS-Koords bekommen
			 * haben.
			 */
			...
			// boolean caseC =
			// (new_location.getProvider().equals(LocationManager.GPS_PROVIDER));
			//
		...
		public void onStatusChanged(String provider, int new_status,
				Bundle extra) {
		}
	...
	public class SendTweetListener implements OnClickListener {
		...
		public void onClick(View v) {
			String text = ((TextView) findViewById(R.id.tweet_text)).getText()
					.toString().trim();
			...
			tweet.imageHoster = getSharedPreferences(Constants.PREFS_APP, 0)
					.getString("pref_image_hoster", "twitter");
			tweet.imageSize = Long.parseLong(getSharedPreferences(
					Constants.PREFS_APP, 0).getString("pref_image_size", "-1"));
	...
	private void serviceBind() {
		startService(new Intent(this.getApplicationContext(),
				TweetSendService.class));
		bindService(new Intent(this.getApplicationContext(),
				TweetSendService.class), serviceConnection,
