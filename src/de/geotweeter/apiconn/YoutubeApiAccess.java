...
public class YoutubeApiAccess {
	...
	public static Pair<URL, URL> getUrlPair(Url url)
		...
		if (screen_url.getHost().endsWith("youtu.be")) {
			return new Pair<URL, URL>(new URL("http://img.youtube.com/vi"
					+ screen_url.getPath() + "/default.jpg"), new URL(
					"http://img.youtube.com/vi" + screen_url.getPath()
							+ "/maxresdefault.jpg"));
		}
		if (screen_url.getHost().contains("youtube.")) {
			if (screen_url.getPath().startsWith("/video/")) {
				String video_id = Utils.substringAfter(screen_url.getPath(),
						"video/");
				return new Pair<URL, URL>(new URL("http://img.youtube.com/vi/"
						+ video_id + "/default.jpg"), new URL(
